// I also added here the "contact" info this allows the system to store and update the user
...
export class UsersService {
  ...
  async createUser(username: string, password: string, contact: number, role = 'user') {
    const hashed = await bcrypt.hash(password, 10);
    const [result] = await this.pool().execute<OkPacket>(
      'INSERT INTO users (username, password, contact, role) VALUES (?, ?, ?, ?)',
      [username, hashed, contact, role],
    );
    return { id: result.insertId, username, contact, role };
  }
